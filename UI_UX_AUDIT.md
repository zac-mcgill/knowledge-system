...
## 23. Phase 30D2 Implementation Note

Phase 30D2 (2026-05-13) is the second sub-slice of Phase 30D. It rebuilds `/app/notes` and `/app/graph` as split-pane workspaces on the Phase 30B primitives. Concretely:

- `/app/notes` is rewritten around a `cve-workbench` split-pane shell. The header is a `cve-toolbar` with the vault selector, Validation / Tasks shortcuts, and Refresh. Status messages surface in a single `cve-banner` whose severity is derived from vault and notes state. The rail hosts filters (text, status, difficulty, missing sections, imported-only with `data-testid="filter-imported-only"`, draft-trust-only with `data-testid="filter-draft-trust-only"`), the POST `/query` search disclosure, and a deterministic note list inside a `cve-scroll-region`. The inspector shows note frontmatter, a section outline, full markdown body (read-only or edit textarea), Trust+Import panel (`data-testid="trust-import-panel"`), validation context, and the per-note improvement task, all inside its own `cve-scroll-region`. Imported and trust-level badges (`data-testid="badge-imported"`, `data-testid="badge-trust"`), the `?vault=` / `?filter=` / `?path=` deep-link contract, and the safe edit workflow (`updateNote`) are preserved.
- `/app/graph` is rewritten around a `cve-workbench` split-pane shell. The legacy `'graph' | 'inspector' | 'missing'` tab model is removed. The rail hosts node-type and edge-type filter checkboxes, a node search input, and a grouped deterministic node list inside a `cve-scroll-region`. The inspector either surfaces the ranked missing concepts overview (default view when no node is selected, with the deterministic ranked table and the non-destructive copyable Action Card disclosed as secondary content) or, when a node is selected, the node header followed by Neighbours, Related notes (with a minimum-strength selector), and Missing concepts near this node, all inline (no sub-tabs) inside a `cve-scroll-region`.
...
- `ui/src/styles/global.css` gained a Phase 30D2 primitive block defining rail, inspector, list, badge, table, outline, edit-grid, task, related-controls, action-card, and link-button helpers (`.cve-p30d2-*`). All declarations use `var(--cve-*)` tokens only; no Tailwind dark palette literals were introduced.
- `mcp/test_verify.py` adds 24 deterministic guardrail tests; total test count rises from 842 to 866.

Remaining Phase 30D pages (Import, Bundles, Exports, Security) ship in Phase 30D3. Phase 30E and Phase 30F remain planned.

## 24. Phase 30D3 Implementation Note

Phase 30D3 (2026-05-22) is the third and final sub-slice of Phase 30D. It rebuilds `/app/import`, `/app/bundles`, `/app/exports`, and `/app/security` on top of the Phase 30B primitives. Concretely:

- All four pages now mount on `AppLayout layoutMode="wide"` and use a `cve-toolbar` header with a state pill, sectioned `cve-p30d3-section` cards, `cve-banner` for status messages, and `cve-status-strip` summary tiles. Raw responses are confined to `cve-details--inspector` blocks with a `cve-details__developer-link` to `/app/raw?endpoint=<name>&vault=<v>&source=<page>`.
- `/app/import` keeps the Phase 26 source-type contract (markdown + obsidian only) and every related testid (source-type-select, source-type-help, summary-wikilinks, summary-embeds, summary-attachments, empty-items-message, collision-banner, frontmatter-banner, item-errors, obsidian-metadata). Preview and Write are separate buttons. Write is disabled until a successful preview, behind a confirmation checkbox (`import-confirm-checkbox`), and is re-disabled when form input changes (`import-stale-banner`). Post-write rendering reuses the existing `ImportedReviewSummary`. Follow-up links to `/app/notes`, `/app/validation`, `/app/tasks`, `/app/trust`, `/app/security` are surfaced.
- `/app/bundles` is a two-column workflow with sectioned form (Scope / Filters / Sections / Budget), a sticky `cve-p30d3-sticky-action` Generate Preview button, and a state-aware right pane (Readiness stages until generation, then a `cve-status-strip` summary plus follow-up links). State pill (`bundle-state-pill`) reflects idle/loading/ok/error.
- `/app/exports` shares the bundle form via the new `ui/src/lib/bundleConfig.ts` helper. The security gate (`export-require-security-checkbox`) defaults to ON. Overwrite (`export-overwrite-checkbox`) reveals a typed-confirmation block (`export-overwrite-confirm-block`); Submit (`export-submit-btn`) only enables once the input matches `OVERWRITE`. Files manifest renders in a bounded `cve-table` with sha256, bytes, and dest columns. Exports stays on its own route distinct from Bundles.
- `/app/security` defaults to a full-vault scan. `fetchNotes(vault)` surfaces a pre-run note count (`security-prerun-tile`) so users see what the deterministic scan will cover. Sampling, filters, sections, allow_partial, max_notes, and max_chars are demoted to an Advanced scope `<details class="cve-p30d3-disclosure">` (`security-advanced-summary`) that is closed by default. Findings render in a bounded `cve-table` inside `cve-p30d3-findings-table` with severity / rule / path / field / detail columns plus severity-segmented filter and free-text filter.
- `ui/src/styles/global.css` gained a Phase 30D3 primitive block defining workflow, twocol, section, field, checkbox, segmented, chip-list, sticky-action, action-row, table-wrap, item-list, tag, readiness, stage-list, summary-kv, mono, divider, followup, toolbar-pill, confirm-block, disclosure, progress, empty, findings-table, findings-detail, btn-success, and btn-danger helpers (`.cve-p30d3-*`). All declarations use `var(--cve-*)` tokens only; no Tailwind dark palette literals were introduced. No backend route, schema, MCP, or runtime dependency was added.
- `mcp/test_verify.py` adds 24 deterministic guardrail tests; total test count rises from 866 to 890. The parent Phase 30D is now Complete. Phase 30E and Phase 30F remain planned.


## 25. Phase 30E1 Implementation Note

Phase 30E1 (2026-05-23) is the first sub-slice of Phase 30E - Review, Governance, and Developer Polish. It rebuilds /app/pending, /app/trust, and /app/feedback on top of the Phase 30B primitives without touching backend routes, schemas, MCP, or runtime dependencies.

- All three pages now use a `cve-toolbar` header with a state pill, vault select, refresh, and a Developer deep-link to `/app/raw?endpoint=<name>&vault=<v>&source=<page>` built by a new shared helper at `ui/src/lib/phase30e1.ts` (uildRawDeepLink, severityWeight, isResolvedSignal, isConfirmed, accept/reject phrases).
- `/app/pending` uses `cve-workbench` (rail + inspector) with internal-scroll testids (pending-queue-scroll, pending-inspector-scroll). The inspector renders a provenance `<dl>` (pending-provenance), a trust impact summary linked to `/app/trust` (pending-trust-impact), a hash warning (pending-hash-warning), and a `cve-diff` (pending-diff). Accept and Reject panes both require typed `ACCEPT` / `REJECT` confirmation; the submit buttons (pending-accept-submit, pending-reject-submit) stay disabled until the confirmation text matches.
- `/app/trust` keeps the `cve-trust-warning` disclaimer and leads with a stale and low-trust governance queue `cve-table` (	rust-governance-table) whose rows expose Notes, Pending, and Feedback links per row (	rust-row-notes, 	rust-row-pending, 	rust-row-feedback). The legacy Evidence Builder is demoted into a closed `<details data-testid="trust-evidence-builder">` disclosure.
- `/app/feedback` renders a labelled filter rail (eedback-filter-severity, eedback-filter-signal, eedback-filter-source, eedback-filter-resolved, eedback-filter-text) and a `cve-table` triage view (eedback-triage-table) sorted via the deterministic severityWeight helper. `Add Feedback` is in the toolbar (eedback-add-action) and opens a `cve-slide-over` (eedback-add-slide-over). The Improvement Tasks panel (eedback-tasks-panel) has a deterministic empty state.
- `ui/src/styles/global.css` gained a Phase 30E1 primitive block defining `cve-p30e1-*` helpers (page, pill, mono, rail, queue list/item, tags by status/severity, inspector, validation, sections, kv, trust-impact, stale, validation-list, diff, action-row, decision panes, twocol layout, side-panel, edit-form, task-list, checkbox-row, evidence-disclosure). All declarations use `var(--cve-*)` tokens; no Tailwind dark palette literals were introduced.
- `mcp/test_verify.py` adds 23 deterministic guardrail tests; total test count rises from 890 to 913.

### Phase 30E2 - Controller and Vault Setup Polish (2026-05-13)

Phase 30E2 closes out Phase 30E - Review, Governance, and Developer Polish. It rebuilds `/app/controller` and `/app/vault-setup` on top of the Phase 30B primitives without touching backend routes, schemas, MCP, or runtime dependencies.

- `/app/controller` adopts a two-column command-centre layout at xl+ (`controller-state-column` left, `controller-recommendation-column` right). The page now uses `cve-toolbar`, a state pill (`controller-state-pill`), `cve-banner`, and `cve-status-strip`. Readiness polarity is corrected via `readinessPolarity()` so negative flags (`has_tasks`, `has_missing_concepts`, `has_feedback_warnings`) no longer render as positive. Recommendations sort deterministically and deep-link to authoritative `/app/*` routes through `recommendationRoute()`. Raw controller and plan responses are demoted to a `cve-details--inspector` disclosure with Developer deep-links into `/app/raw` built by `buildRawDeepLink()`.
- `/app/vault-setup` collapses scattered per-field cards into one grouped `cve-p30e2-form-grid` panel and keeps live validation plus the bootstrap preview. Destructive vault deletion is fully removed from the primary setup form and relocated into a dedicated vault management panel; the actual delete flow runs inside a `cve-slide-over` (`vault-setup-delete-slide-over`) that names the target vault, explains the real backend semantics (files deleted from disk, `config/config.yaml` rewritten, action not reversible by the app) sourced from `VAULT_DELETE_SEMANTICS`, and requires a typed `DELETE <vault>` confirmation enforced by `isDeleteConfirmed()`. `demo-vault` is protected via `VAULT_DELETE_PROTECTED` and its delete trigger is disabled.
