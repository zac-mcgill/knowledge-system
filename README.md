...
**Local-first Python pipeline: credential leak scanning, prompt-injection detection, schema enforcement, rate-limited API, path-traversal blocking, SHA-256 artefact integrity, MCP stdio compatibility layer with documented client setup, a local MCP Setup UI page, a deterministic local connection smoke test, and a completed MCP stdio verification batch pass (Phase 39A, documentation-only, no runtime changes), private cloud mode, session and project state, safe memory write queue, trust/staleness/evidence metadata, safe Markdown folder import with browser review UI, post-import review integration, hardened import edge-case handling, Obsidian-compatible Markdown import, end-to-end import lifecycle finalisation, and a local, redacted diagnostics and support report, and Phase 38 local backup, restore, and migration safety (preview-first restore with typed confirmation, generated artefacts excluded by default). 1166 tests.**
...
- Phases 0 to 31C are complete. Phase 39A (MCP Stdio Verification Batch Pass) is complete and documented: a manual/copilot-assisted 12-batch verification pass validated the MCP stdio surface, catalogue, error handling, pending-change safety, documentation, UI build, boundary separation, artefact hygiene, and final release gate. No runtime code changes, no source modifications, and no commit were required by the verification itself. Phase 32 (Human Release QA and Evidence Capture) is the next planned phase; Phases 32 to 44 are planned. Phase 31C performed automated source-level verification only: the agent that executed Phase 31C did not open a browser, did not perform live keyboard traversal, and did not run a screen reader. Manual browser visual QA, live keyboard QA, and screen-reader QA remain manual unless actually performed.
- The local app, CLI, HTTP API, and MCP stdio surface are release-candidate quality for local use, pending manual browser visual QA, keyboard QA, and screen-reader QA.
- 1166 deterministic tests cover phases 0 to 25, the Phase 26A import pipeline, Phase 26B import review UI, Phase 26C post-import review integration, Phase 26D edge-case hardening, Phase 26E Obsidian-compatible import, Phase 26F end-to-end import lifecycle finalisation, Phase 29A documentation guardrails, Phase 29B navigation and information architecture guardrails, Phase 29C design system foundation guardrails, Phase 29D page-level UX consistency guardrails, Phase 29E final polish and release-readiness guardrails, Phase 30B app-shell / theme / primitive foundation guardrails, Phase 30C Dashboard redesign guardrails, Phase 30D1 Validation, Tasks, and Raw Developer real-implementation guardrails, Phase 30D2 Notes and Graph workspace redesign guardrails, Phase 30D3 Security and Imports redesign guardrails, Phase 30E1 Pending/Trust/Feedback governance polish guardrails, Phase 30F final QA, light-mode toggle, accessibility, and responsive guardrails (source-level only; browser visual and screen-reader QA remain manual), Phase 31A release-candidate verification checklist guardrails, Phase 31B header/toolbar normalisation guardrails, and Phase 31C release-candidate visual QA and defect-triage documentation-honesty guardrails (source-level only; the agent that executed Phase 31C did not open a browser, did not perform live keyboard traversal, and did not run a screen reader, so the manual rows in `RELEASE_CHECKLIST.md` remain manual), plus documentation drift guardrails.
...
- MCP stdio compatibility for vault inspection, deterministic context planning, and queued pending-change proposals reviewed by a human (no direct vault-note write path, no autonomous accept, no semantic retrieval, no embeddings, no LLM calls)
...
- Post-Import Review Integration (Phase 26C): after a successful import, the Import Review page renders an Imported Review Summary (imported total, imported drafts, imported notes with validation issues, imported notes with tasks, imported stale, imported deprecated) and vault-aware follow-up links into Notes (with `imported`, `draft`, and `imported-draft` filters), Validation, Tasks, Trust, Security, and the dashboard. The Notes page surfaces `source_type` and `trust_level` on every row, adds Imported-only and Draft-trust-only filters, and shows a Trust and Import panel in the note detail (source_type, trust_level, last_reviewed, review_after, confidence, stale) along with the disclaimer that trust metadata reflects review and maintenance state only and does not prove factual correctness. No automatic trust promotion, no automatic LLM rewriting, no new import sources. Obsidian-compatible Markdown import is implemented (Phase 26E); PDF, browser article, GitHub repo, chat transcript, semantic mapping, and LLM-extraction imports remain deferred, and full Obsidian-native behaviour (automatic wikilink rewriting, attachment copying/import, binary attachment processing) remains deferred.
- Import Workflow Hardening (Phase 26D): tightens the Markdown import pipeline against real-world edge cases (orphan YAML opening markers, duplicate YAML keys, non-object frontmatter, null bytes, oversize files, nested source folders, duplicate filenames, punctuation/empty filename stems, Windows backslash destinations) and surfaces the failure modes with item-level error codes (`READ_FAILED`, `SOURCE_TOO_LARGE`, `NULL_BYTE`, `INVALID_FRONTMATTER`, `FRONTMATTER_NOT_OBJECT`, `DUPLICATE_YAML_KEY`, `DESTINATION_EXISTS`, `UNSAFE_DESTINATION`, `SECURITY_FAIL`, `VALIDATION_FAILED`, `WRITE_FAILED`). One bad file no longer crashes the batch, dry-run is deterministic, summary counts match item statuses, and the Import Review UI surfaces collision and malformed-frontmatter cases. Phase 26D adds no new import sources, no semantic mapping, no LLM extraction, and no automatic trust promotion; Obsidian-compatible Markdown import was added in Phase 26E, while PDF, browser article, GitHub repo, chat transcript, semantic mapping, and LLM-extraction imports remain deferred, and full Obsidian-native behaviour remains deferred.
...
| **MCP** | Exposes vault capabilities as JSON-RPC tools, resources, and prompts over stdio for use with MCP-compatible local clients. Most tools are deterministic read-only queries; a small number queue pending-change proposals for human review. No direct vault-note write path, no autonomous accept, no semantic retrieval, no embeddings, no LLM calls. See `QUICKSTART.md` section 24 for client setup and the `py run.py mcp-smoke` connection test. |
...
# MCP stdio server (Phase 20, setup hardened in Phase 39)
py run.py mcp                     # start MCP JSON-RPC stdio server
py run.py mcp-smoke               # deterministic local MCP connection smoke test
py run.py diagnostics             # print a local, redacted diagnostics report as JSON
